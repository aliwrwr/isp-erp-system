<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-lg font-bold text-secondary">طھظ‚ط§ط±ظٹط± ط§ظ„ظ…ط·ط¹ظ…</h2>
      ...
      <div class="flex items-center gap-2">
        <button @click="printReport"
          ...
          <span class="hidden sm:inline">ط·ط¨ط§ط¹ط©</span>
        </button>
        <button @click="load"
          :disabled="loading"
          ...
          <span class="hidden sm:inline">طھط­ط¯ظٹط«</span>
        </button>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-6">
      <div class="flex flex-wrap items-center gap-3">
        ...
        <div class="flex items-center gap-1.5 flex-wrap">
          <span class="text-xs font-bold text-gray-400 ml-2">ظپطھط±ط© ط²ظ…ظ†ظٹط©:</span>
        ...
        <div class="flex items-center gap-2 flex-wrap">
          <span class="text-xs font-bold text-gray-400">ظ…ظ†:</span>
          ...
          <span class="text-xs font-bold text-gray-400">ط¥ظ„ظ‰:</span>
          ...
          <button @click="applyCustom"
            ...
            <i class="fas fa-search ml-1"></i> ط¨ط­ط«
          </button>
          <button v-if="activeQuick !== 'all'" @click="applyQuick('all')"
            ...
            <i class="fas fa-times"></i> ط¥ظ„ط؛ط§ط،
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-if="loading" class="text-center text-gray-400 py-16">
      ...
      <p class="text-sm">ط¬ط§ط±ظٹ طھط­ظ…ظٹظ„ ط§ظ„طھظ‚ط§ط±ظٹط±...</p>
    </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition">
            <div class="flex items-center gap-2 mb-3">
              ...
              <span class="text-xs text-gray-400 font-medium">ط¥ط¬ظ…ط§ظ„ظٹ ط§ظ„ط·ظ„ط¨ط§طھ</span>
            </div>
            <p class="text-2xl font-bold text-secondary">{{ data.totalOrders }}</p>
            <p class="text-xs text-gray-400 mt-1">ط§ظ„ظٹظˆظ…: <span class="font-bold text-blue-500">{{ data.todayOrders }}</span></p>
          </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition">
            <div class="flex items-center gap-2 mb-3">
              ...
              <span class="text-xs text-gray-400 font-medium">ط·ظ„ط¨ط§طھ ظ…ط¯ظپظˆط¹ط©</span>
            </div>
            <p class="text-2xl font-bold text-green-600">{{ data.paidOrders }}</p>
            <p class="text-xs text-gray-400 mt-1">ظ…ظ„ط؛ظٹط©: <span class="font-bold text-red-400">{{ data.cancelledOrders }}</span></p>
          </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition">
            <div class="flex items-center gap-2 mb-3">
              ...
              <span class="text-xs text-gray-400 font-medium">ط¥ط¬ظ…ط§ظ„ظٹ ط§ظ„ط¥ظٹط±ط§ط¯ط§طھ</span>
            </div>
            <p class="text-2xl font-bold text-restaurant">{{ data.totalRevenue.toFixed(2) }}</p>
            <p class="text-xs text-gray-400 mt-1">ط§ظ„ظٹظˆظ…: <span class="font-bold text-restaurant">{{ data.todayRevenue.toFixed(2) }}</span></p>
          </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition">
            <div class="flex items-center gap-2 mb-3">
              ...
              <span class="text-xs text-gray-400 font-medium">ط¥ط¬ظ…ط§ظ„ظٹ ط§ظ„ظ…طµط±ظˆظپط§طھ</span>
            </div>
            <p class="text-2xl font-bold text-danger">{{ data.totalExpenses.toFixed(2) }}</p>
            <p class="text-xs text-gray-400 mt-1">ط§ظ„ظٹظˆظ…: <span class="font-bold text-danger">{{ data.todayExpenses.toFixed(2) }}</span></p>
          </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition"
            ...
            <div class="flex items-center gap-2 mb-3">
              ...
              <span class="text-xs text-gray-400 font-medium">طµط§ظپظٹ ط§ظ„ط±ط¨ط­</span>
            </div>
            <p class="text-2xl font-bold" :class="data.netProfit >= 0 ? 'text-green-600' : 'text-red-500'">
              {{ data.netProfit.toFixed(2) }}
            </p>
            <p class="text-xs text-gray-400 mt-1">ط§ظ„ظٹظˆظ…: <span class="font-bold" :class="data.todayProfit >= 0 ? 'text-green-500' : 'text-red-400'">{{ data.todayProfit.toFixed(2) }}</span></p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-sm font-bold text-secondary flex items-center gap-2">
                ...
                ط§ظ„ط¥ظٹط±ط§ط¯ط§طھ ط§ظ„ظٹظˆظ…ظٹط©
              </h3>
              <span class="text-[10px] text-gray-400 bg-gray-50 px-2 py-1 rounded-lg">{{ data.dailyData.filter((x: any) => x.revenue > 0 || x.expenses > 0).length }} ظٹظˆظ…</span>
            </div>
            <div class="space-y-1 max-h-[380px] overflow-y-auto custom-scrollbar pl-1">
              ...
              <div v-if="data.dailyData.every((x: any) => x.revenue === 0 && x.expenses === 0)"
                ...
                <p>ظ„ط§ طھظˆط¬ط¯ ط¨ظٹط§ظ†ط§طھ ظپظٹ ظ‡ط°ظ‡ ط§ظ„ظپطھط±ط©</p>
              </div>
            </div>
            <div class="flex items-center gap-4 mt-3 pt-3 border-t border-gray-100 text-xs text-gray-400">
              <span class="flex items-center gap-1.5"><span class="w-3 h-3 rounded-full bg-green-400 inline-block"></span> ط¥ظٹط±ط§ط¯ط§طھ</span>
              <span class="flex items-center gap-1.5"><span class="w-3 h-3 rounded-full bg-red-300 inline-block"></span> ظ…طµط±ظˆظپط§طھ</span>
            </div>
          </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-sm font-bold text-secondary flex items-center gap-2">
                ...
                ط£ظƒط«ط± ط§ظ„ط£طµظ†ط§ظپ ظ…ط¨ظٹط¹ط§ظ‹
              </h3>
              <span class="text-[10px] text-gray-400 bg-gray-50 px-2 py-1 rounded-lg">ط£ظپط¶ظ„ {{ data.topItems.length }}</span>
            </div>
            <div class="space-y-3 max-h-[380px] overflow-y-auto custom-scrollbar">
              <div v-for="(item, idx) in data.topItems" :key="item.name"
                ...
                <span class="w-7 h-7 rounded-xl flex items-center justify-center text-xs font-bold shrink-0"
                  ...
                  {{ (idx as number) < 3 ? ['ًں¥‡','ًں¥ˆ','ًں¥‰'][idx as number] : (idx as number) + 1 }}
                </span>
                ...
                <div class="text-left shrink-0">
                  <p class="text-sm font-bold text-restaurant">{{ item.revenue.toFixed(0) }} <span class="text-[10px] font-normal">ط¯.ط¹</span></p>
                  <p class="text-[10px] text-gray-400">{{ item.quantity }} ظˆط­ط¯ط©</p>
                </div>
              </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            <div class="flex items-center justify-between mb-4">
              ...
              <span class="text-sm font-bold text-danger">{{ data.totalExpenses.toFixed(2) }} <span class="text-[10px] font-normal text-gray-400">ط¯.ط¹</span></span>
            </div>
            <div class="space-y-3">
              ...
              <div v-if="Object.keys(data.expensesByCategory).length === 0"
                ...
                <p>ظ„ط§ طھظˆط¬ط¯ ظ…طµط±ظˆظپط§طھ</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
              <div class="bg-green-50 rounded-2xl p-4 text-center border border-green-100">
                ...
                <p class="text-xs text-green-600 mt-1 font-medium">ظ…ط¯ظپظˆط¹ط©</p>
              </div>
              <div class="bg-red-50 rounded-2xl p-4 text-center border border-red-100">
                ...
                <p class="text-xs text-red-500 mt-1 font-medium">ظ…ظ„ط؛ظٹط©</p>
              </div>
              <div class="bg-blue-50 rounded-2xl p-4 text-center border border-blue-100">
                ...
                <p class="text-xs text-blue-600 mt-1 font-medium">ط·ظ„ط¨ط§طھ ط§ظ„ظٹظˆظ…</p>
              </div>
              <div class="bg-restaurant/5 rounded-2xl p-4 text-center border border-restaurant/10">
                ...
                <p class="text-xs text-restaurant mt-1 font-medium">ط¥ط¬ظ…ط§ظ„ظٹ ط§ظ„ط·ظ„ط¨ط§طھ</p>
              </div>
            </div>
            <div class="space-y-2 pt-3 border-t border-gray-100">
              <div class="flex justify-between items-center py-1.5 px-2 rounded-xl hover:bg-gray-50 transition">
                <span class="text-xs text-gray-500 font-medium">ظ…طھظˆط³ط· ظ‚ظٹظ…ط© ط§ظ„ط·ظ„ط¨</span>
                <span class="text-sm font-bold text-secondary">
                  {{ data.paidOrders > 0 ? (data.totalRevenue / data.paidOrders).toFixed(2) : '0.00' }} <span class="text-[10px] font-normal text-gray-400">ط¯.ط¹</span>
                </span>
              </div>
              <div class="flex justify-between items-center py-1.5 px-2 rounded-xl hover:bg-gray-50 transition">
                <span class="text-xs text-gray-500 font-medium">ظ†ط³ط¨ط© ط§ظ„ط¥ظ„ط؛ط§ط،</span>
                <span class="text-sm font-bold" :class="cancelRate > 20 ? 'text-red-500' : 'text-green-600'">
                  {{ cancelRate.toFixed(1) }}%
                </span>
              </div>
              <div class="flex justify-between items-center py-1.5 px-2 rounded-xl hover:bg-gray-50 transition">
                <span class="text-xs text-gray-500 font-medium">ظ‡ط§ظ…ط´ ط§ظ„ط±ط¨ط­</span>
    ...
    <div v-else-if="!loading" class="text-center text-gray-300 py-16">
      ...
      <p class="text-lg font-medium">ظ„ط§ طھظˆط¬ط¯ ط¨ظٹط§ظ†ط§طھ</p>
      <p class="text-sm mt-1">ط§ط¶ط؛ط· طھط­ط¯ظٹط« ظ„طھط­ظ…ظٹظ„ ط§ظ„طھظ‚ط§ط±ظٹط±</p>
    </div>
  </div>
</template>
...
const quickFilters = [
  { key: 'today',   label: 'ط§ظ„ظٹظˆظ…' },
  { key: 'yesterday', label: 'ط§ظ„ط£ظ…ط³' },
  { key: 'week',    label: 'ظ‡ط°ط§ ط§ظ„ط£ط³ط¨ظˆط¹' },
  { key: 'month',   label: 'ظ‡ط°ط§ ط§ظ„ط´ظ‡ط±' },
  { key: 'last3',   label: 'ط¢ط®ط± 3 ط£ط´ظ‡ط±' },
  { key: 'all',     label: 'ط§ظ„ظƒظ„' },
];

const rangeLabel = computed(() => {
  if (activeQuick.value === 'today') return 'ط¹ط±ط¶: ط§ظ„ظٹظˆظ…';
  if (activeQuick.value === 'yesterday') return 'ط¹ط±ط¶: ط§ظ„ط£ظ…ط³';
  if (activeQuick.value === 'week') return 'ط¹ط±ط¶: ظ‡ط°ط§ ط§ظ„ط£ط³ط¨ظˆط¹';
  if (activeQuick.value === 'month') return 'ط¹ط±ط¶: ظ‡ط°ط§ ط§ظ„ط´ظ‡ط±';
  if (activeQuick.value === 'last3') return 'ط¹ط±ط¶: ط¢ط®ط± 3 ط£ط´ظ‡ط±';
  if (dateFrom.value || dateTo.value) return `ط¹ط±ط¶: ${dateFrom.value || '...'} â†’ ${dateTo.value || '...'}`;
  return 'ط¹ط±ط¶: ط¬ظ…ظٹط¹ ط§ظ„ط¨ظٹط§ظ†ط§طھ';
});

const maxRevenue = computed(() =>
  data.value ? Math.max(...data.value.dailyData.map((d: any) => Math.max(d.revenue, d.expenses)), 1) : 1
);
...
function barWidth(val: number, max: number) { return max > 0 ? Math.min(val / max * 100, 100) : 0; }
...
function expCatClass(c: string) {
  return ({ ingredients: 'bg-green-50 text-green-700', maintenance: 'bg-blue-50 text-blue-700', salaries: 'bg-purple-50 text-purple-700', utilities: 'bg-yellow-50 text-yellow-700', general: 'bg-gray-100 text-gray-600' } as any)[c] || 'bg-gray-100 text-gray-600';
}
function expCatLabel(c: string) {
  return ({ ingredients: 'ظ…ظˆط§ط¯ ط£ظˆظ„ظٹط©', maintenance: 'طµظٹط§ظ†ط©', salaries: 'ط±ظˆط§طھط¨', utilities: 'ط®ط¯ظ…ط§طھ', general: 'ط¹ط§ظ…' } as any)[c] || c;
}
function expCatBar(c: string) {
  return ({ ingredients: 'bg-green-400', maintenance: 'bg-blue-400', salaries: 'bg-purple-400', utilities: 'bg-yellow-400', general: 'bg-gray-400' } as any)[c] || 'bg-gray-400';
}

function printReport() {
  const content = document.getElementById('print-area')?.innerHTML || '';
  const title = `طھظ‚ط§ط±ظٹط± ط§ظ„ظ…ط·ط¹ظ… â€” ${rangeLabel.value}`;
