      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="m in history" :key="m.id" class="border-t border-gray-50 hover:bg-gray-50">
            ...
            <td class="px-4 py-3"><span class="px-2 py-1 rounded-full text-xs font-medium" :class="m.status === 'delivered' ? 'bg-green-100 text-green-700' : m.status === 'failed' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'">{{ {delivered:'تم التوصيل',failed:'فشل',pending:'قيد الإرسال'}[m.status] }}</span></td>
          </tr>
        </tbody>
      </table>
    </DataTable>
